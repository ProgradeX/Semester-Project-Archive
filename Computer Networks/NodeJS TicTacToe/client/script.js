...
function renderTurnMessage() {
    if (!myTurn) { // If not player's turn disable the board
        $("#announcer").text("Your opponent's turn");
        $(".board button").attr("disabled", true);
    } else { // Enable it otherwise
        $("#announcer").text("Your turn.");
...
socket.on("opponent.left", function() {
    $("#announcer").text("Your opponent left the game.");
    $(".board button").attr("disabled", true);
});

// Binding buttons on the board
$(function() {
  $(".gridbutton").attr("disabled", true); // Disable board at the beginning
  $(".gridbutton").on("click", makeMove);
});
